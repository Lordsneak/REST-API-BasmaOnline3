...
public class CategorieServiceImpl implements CategorieService {
	...
	@Override
	public List<CategorieDto> getCategories(int page, int limit) {

		if(page > 0) page = page -1;
		
		List<CategorieDto> categorysDto = new ArrayList<>();
		
		Pageable pageableRequest = PageRequest.of(page, limit);
		
		Page<CategorieEntity> categoryPage =	categorieRepository.findAll(pageableRequest);
		
		List<CategorieEntity> categorys = categoryPage.getContent();
		
		for(CategorieEntity categoryEntity: categorys) {
			CategorieDto category = new CategorieDto();
